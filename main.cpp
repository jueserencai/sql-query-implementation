...
// 预先读入两个table的所有row，再join等等。只有一遍读磁盘操作，所以速度更快了。变成 615毫秒。多刷几次，最好为588毫秒，最差为680毫秒
...
struct SortByComparator {
    bool operator()(const Group *group1, const Group *group2) const {
        return group1->t1_id1 < group2->t1_id1 || (group1->t1_id1 == group2->t1_id1 && group1->t2_id2 < group2->t2_id2) || (group1->t2_id2 == group2->t2_id2 && group1->t1_id2 < group2->t1_id2);
    }
};
...
std::unordered_multimap<int, Row *> read_rows_build_index(std::string filename) {
...
std::vector<std::pair<Row *, Row *>> join(std::string file1_name, std::string file2_name) {
...
#endif
    auto t1_rows = read_rows(file1_name);
    auto t2_rows = read_rows(file2_name);
    std::unordered_multimap<int, Row *> id3_index;
    for (auto it = t1_rows.begin(); it != t1_rows.end(); it++) {
        id3_index.insert({(*it)->id3, (*it)});
    }
    ...
    std::vector<std::pair<Row *, Row *>> join_rows;
    for (auto t2_it = t2_rows.begin(); t2_it != t2_rows.end(); t2_it++) {
        auto range = id3_index.equal_range((*t2_it)->id3);
        for (auto range_it = range.first; range_it != range.second; range_it++) {
            join_rows.push_back({range_it->second, (*t2_it)});
        }
    }
...
std::vector<Group *> group(std::vector<std::pair<Row *, Row *>> &join_rows) {
    ...
    // std::map<std::pair<int, int>, Group *> groups_index;
    std::unordered_map<int, std::unordered_map<int, Group *>> groups_index;
    for (auto join_it = join_rows.begin(); join_it != join_rows.end(); join_it++) {
        auto key1 = (*join_it).first->id2;
        auto key2 = (*join_it).second->id2;
        if (groups_index.count(key1) == 0) {
            groups_index[key1] = std::unordered_map<int, Group *>();
        }
        if (groups_index[key1].count(key2) == 0) {
            auto group = new Group();
            group->t1_id1 = (*join_it).first->id1;
            group->t1_id2 = (*join_it).first->id2;
            group->t2_id1 = (*join_it).second->id1;
            group->t2_id2 = (*join_it).second->id2;
            groups_index[key1][key2] = group;
        } else {
            auto group = groups_index[key1][key2];
            group->t1_id1 = std::max(group->t1_id1, (*join_it).first->id1);
            group->t2_id1 = std::min(group->t2_id1, (*join_it).second->id1);
        }
    }
    ...
    for (auto key1_it = groups_index.begin(); key1_it != groups_index.end(); key1_it++) {
        for (auto key2_it = key1_it->second.begin(); key2_it != key1_it->second.end(); key2_it++) {
            groups.push_back(key2_it->second);
        }
    }
