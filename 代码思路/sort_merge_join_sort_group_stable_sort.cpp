...
std::vector<std::pair<Row *, Row *>> sort_merge_join(std::vector<Row *> &left_relation,
    ...
    std::sort(left_relation.begin(), left_relation.end(), SortMergeComparator());    // 按照id3排序
    std::sort(right_relation.begin(), right_relation.end(), SortMergeComparator());  // 按照id3排序

    auto &left_sorted = left_relation;
    auto &right_sorted = right_relation;

    auto left_it_begin = left_sorted.begin();    // left_relation 区间的左边界
    auto left_it_end = left_sorted.begin();      // left_relation 区间的右边界。区间内部是相等的id3
    auto right_it_begin = right_sorted.begin();  // right_relation 区间的左边界
    auto right_it_end = right_sorted.begin();    // right_relation 区间的右边界。区间内部是相等的id3
    auto left_sorted_end = left_sorted.end();
    auto right_sorted_end = right_sorted.end();

    advance(left_it_begin, left_it_end, left_sorted_end);     // 找到第一个区间
    advance(right_it_begin, right_it_end, right_sorted_end);  // 找到第一个区间

    while (left_it_begin != left_it_end && right_it_begin != right_it_end) {  // 当两个表的区间都不空。可能存在相等的id3。
        JoinColmunType left_key = (*left_it_begin)->id3;
        JoinColmunType right_key = (*right_it_begin)->id3;

        if (left_key == right_key) {  // 满足join谓语，两个记录的id3相等
...
int main() {
    ...
    std::stable_sort(groups.begin(), groups.end(), SortByComparator());  // 按照 max(t1.id1)稳定排序。
