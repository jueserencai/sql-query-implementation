...
struct SortByComparator {
    bool operator()(const Group *group1, const Group *group2) const {
        return group1->t1_id1 < group2->t1_id1
                || (group1->t1_id1 == group2->t1_id1 && group1->t2_id2 < group2->t2_id2)
                || (group1->t2_id2 == group2->t2_id2 && group1->t1_id2 < group2->t1_id2);
    }
};
...
std::vector<Group *> group(std::vector<std::pair<Row *, Row *>> &join_rows) {
    ...
    for (auto join_it = join_rows.begin(); join_it != join_rows.end(); join_it++) {
        t1_id2 = (*join_it).first->id2;
        t2_id2 = (*join_it).second->id2;
        if (groups_index.find(t1_id2) == groups_index.end()) {
            groups_index[t1_id2] = std::unordered_map<int, Group *>();
        }
        if (groups_index[t1_id2].find(t2_id2) == groups_index[t1_id2].end()) {
            auto group = new Group();
            group->t1_id1 = (*join_it).first->id1;
            group->t1_id2 = t1_id2;
            group->t2_id1 = (*join_it).second->id1;
            group->t2_id2 = t2_id2;
            groups_index[t1_id2][t2_id2] = group;
        } else {
            auto group = groups_index[t1_id2][t2_id2];
            group->t1_id1 = std::max(group->t1_id1, (*join_it).first->id1);
            group->t2_id1 = std::min(group->t2_id1, (*join_it).second->id1);
        }
    }

    for (auto t1_id2_it = groups_index.begin(); t1_id2_it != groups_index.end(); t1_id2_it++) {
        for (auto t2_id2_it = t1_id2_it->second.begin(); t2_id2_it != t1_id2_it->second.end(); t2_id2_it++) {
            groups.push_back(t2_id2_it->second);
        }
    }
...
#endif
    // sort
    std::sort(groups.begin(), groups.end(), SortByComparator());
